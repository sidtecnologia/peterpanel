    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {sortedOrders.length === 0 ? (
          <p className="text-center text-slate-400 py-10">Todo despachado.</p>
        ) : (
          sortedOrders.map(o => (
              ...
              <div className="flex flex-col md:flex-row justify-between gap-4">
                <div>
                  <div className="flex gap-2 mb-1">
                    <span className="text-xs font-bold bg-orange-100 text-orange-800 px-2 rounded">
                  ...
                  {o.observation && (
                    <div className="bg-yellow-50 text-yellow-800 p-2 rounded text-sm mb-2 mt-1">
                      Note: {o.observation}
                    </div>
                  )}
                  <div className="text-sm text-slate-600">
                    {typeof o.order_items === 'string' 
                      ? JSON.parse(o.order_items).map(i=>`${i.qty}x ${i.name}`).join(', ') 
                      : (o.order_items||[]).map(i=>`${i.qty}x ${i.name}`).join(', ')}
                  </div>
                  {o._origin === 'table' && (
                    <div className="mt-2 text-xs text-slate-500">
                      Origen: Mesa {o.customer_address}
                    </div>
                  )}
                </div>
