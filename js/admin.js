...
const convertToCSV = (data) => {
    ...
    const rows = data.map(obj => exportFields.map(field => {
        let val = obj[field];
        if (field === 'total_amount' && typeof val === 'number') val = val.toLocaleString('es-CO');
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (response.ok) {
                ...
                // Intenta parsear texto si no es JSON, o si es 204 sin contenido.
...
const initUserView = (role) => {
    ...
    if (userInfo) userInfo.textContent = `Rol: ${role.toUpperCase()}`;
    ...
    switch (role) {
        case 'caja':
            showCajaSection('mesas'); // CAMBIO: Inicia en la sección de mesas
            document.getElementById('caja-btn-mesas').onclick = () => showCajaSection('mesas'); // NUEVO Listener
...
const showCajaSection = (section) => {
    const mesaSection = document.getElementById('caja-section-mesas'); // NUEVO
    ...
    const btnMesa = document.getElementById('caja-btn-mesas'); // NUEVO
...
const isTableSessionTable = (session) => {
    if (!session) return false;

   
    const addr = session.customer_address ?? session.customerAddress ?? null;
    if (addr != null && addr !== '') {
        if (typeof addr === 'number' && Number.isInteger(addr)) {
    ...
    // Fallback a table_number
    const tn = session.table_number ?? session.table ?? null;
    if (tn != null && tn !== '') {
        if (typeof tn === 'number' && Number.isInteger(tn)) {
            return tn >= 0 && tn <= 99;
        }
        const s2 = String(tn).trim();
        if (/^\d{1,2}$/.test(s2)) return true;
    }
...
const loadActiveTables = async () => {
    ...
    const url = `${BASE_API_URL}/table_sessions?select=*&status=eq.open&order=opened_at.asc`;

    try {
        let tables = await makeRequest(url);
        if (!Array.isArray(tables)) tables = [];

        
        const filtered = tables.filter(t => isTableSessionTable(t));

        activeTablesData = filtered;

        const cardsHTML = generateTableCards(activeTablesData);
        const el = document.getElementById('active-tables-list');
        if (el) el.innerHTML = cardsHTML;
        attachTableListeners();
    } catch (e) {
        const el = document.getElementById('active-tables-list');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar mesas activas: ${e.message}</p>`;
        console.error("Error al cargar mesas activas:", e.message);
    }
};
...
const generateTableCards = (data) => {
    ...
    if (data.length === 0) {
        return `<p class="text-gray-600">No hay mesas ocupadas en este momento.</p>`;
    }

    return data.map(table => {
        const totalItems = (Array.isArray(table.items) ? table.items.reduce((sum, item) => sum + (item.qty || 0), 0) : 0);
        let openedTime = '';
        try { openedTime = new Date(table.opened_at).toLocaleTimeString(); } catch(_) { openedTime = ''; }

        // Mostrar table_number si existe, sino intentar derivarlo de customer_address
        const displayNumber = table.table_number ?? table.table ?? table.customer_address ?? 'N/A';

        return `
            <div class="bg-white border-2 border-green-200 p-5 rounded-xl shadow-lg hover:shadow-xl transition duration-150">
                <div class="flex justify-between items-center mb-3 border-b pb-2">
                    <h4 class="text-2xl font-extrabold text-green-700">MESA #${displayNumber}</h4>
                    <span class="text-sm font-medium text-gray-500">${openedTime}</span>
                </div>
                <p class="text-lg font-semibold mb-2">Total Consumo: <span class="text-red-600">$${(table.total || 0).toLocaleString('es-CO')}</span></p>
                <p class="text-sm text-gray-700">Cliente Inicial: ${table.customer_name || 'N/A'}</p>
                <p class="text-sm text-gray-700">Items Pedidos: ${totalItems}</p>
                <div class="mt-4 flex gap-2">
                    <button data-id="${table.id}" data-action="view-table" class="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-150 text-sm">Ver Detalle</button>
                    <button data-id="${table.id}" data-action="close-table" class="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition duration-150 text-sm">Cerrar y Cobrar</button>
                </div>
            </div>
        `;
    }).join('');
...
const attachTableListeners = () => {
    document.querySelectorAll('#active-tables-list button[data-action]').forEach(button => {
        button.onclick = async () => {
            const id = button.getAttribute('data-id');
            const action = button.getAttribute('data-action');
            const table = activeTablesData.find(t => t.id == id);
            
            if (!table) {
                logError("Mesa no encontrada.");
                return;
            }
            ...
            if (action === 'view-table') {
                ...
                const confirmClose = confirm(`¿Seguro que desea cerrar la cuenta de la MESA #${table.table_number || table.customer_address}`);
                if (confirmClose) {
                    await closeTableSession(id, table.total);
                }
            }
        };
    });
...
const showTableDetailModal = (table) => {
    document.getElementById('modal-title').textContent = `Cuenta Detallada - Mesa #${table.table_number ?? table.customer_address ?? ''}`;
    
    
    let itemsHtml = (Array.isArray(table.items) ? table.items : []).map(item => {
        ...
        return `
            <li class="py-1 border-b border-gray-100">
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold">$${itemTotal.toLocaleString('es-CO')}</span>
                </div>
            </li>
        `;
    }).join('');
    ...
    document.getElementById('modal-body').innerHTML = `
        <div class="space-y-2">
            <p><span class="font-semibold">Cliente Inicial:</span> ${table.customer_name || 'N/A'}</p>
            <p><span class="font-semibold">Abierta desde:</span> ${new Date(table.opened_at).toLocaleString()}</p>
        </div>
        
        <h4 class="font-bold mt-4 mb-2 text-lg border-b">Consumo Acumulado:</h4>
        <ul class="list-none pl-0 mb-4">${itemsHtml}</ul>
        
        <div class="text-right font-extrabold text-xl pt-2 border-t mt-4">
            <p class="mt-1">TOTAL A COBRAR: <span class="text-green-600">$ ${total.toLocaleString('es-CO')}</span></p>
        </div>
    `;
    document.getElementById('modal-actions').innerHTML = `
        <button data-id="${table.id}" data-total="${total}" id="print-table-btn" class="px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition duration-150">Imprimir Cuenta</button>
        <button data-id="${table.id}" data-total="${total}" id="close-table-modal-btn" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition duration-150">Cerrar y Cobrar</button>
    `;
    
    document.getElementById('print-table-btn').onclick = () => printTable(table);

    document.getElementById('close-table-modal-btn').onclick = async (e) => {
        const id = e.target.getAttribute('data-id');
        const total = e.target.getAttribute('data-total');
        const confirmClose = confirm(`¿Confirma el cobro de $${parseFloat(total).toLocaleString('es-CO')} para la MESA #${table.table_number ?? table.customer_address}?`);
        if (confirmClose) {
            await closeTableSession(id, total);
            setView('modal', false);
        }
    };
...
const printTable = (table) => {
    ...
    let itemsText = (Array.isArray(table.items) ? table.items : []).map(item =>
        `<div>(${item.qty || item.quantity || 0}x) ${item.name} -> $${((item.price || 0) * (item.qty || item.quantity || 0)).toLocaleString('es-CO')}</div>`
    ).join('');

    printArea.innerHTML = `
        <div class="text-center font-bold mb-2">CUENTA - MESA #${table.table_number ?? table.customer_address ?? ''}</div>
        <div class="mb-2">--------------------------------</div>
        <div>Cliente Inicial: ${table.customer_name || 'N/A'}</div>
        <div>Abierta: ${new Date(table.opened_at).toLocaleString()}</div>
        <div>--------------------------------</div>
        <div class="font-bold">CONSUMO DETALLADO:</div>
        ${itemsText}
        <div>--------------------------------</div>
        <div class="text-right font-bold">TOTAL A PAGAR: $${(table.total || 0).toLocaleString('es-CO')}</div>
        <div class="mt-4 text-center">¡Gracias por su consumo!</div>
    `;
...
const closeTableSession = async (sessionId, total) => {
    try {
        
        await makeRequest(`${BASE_API_URL}/table_sessions?id=eq.${sessionId}`, {
            method: 'PATCH',
            body: JSON.stringify({ 
                status: "closed",
        ...
        console.log(`Mesa ${sessionId} cerrada con éxito por un total de $${parseFloat(total).toLocaleString('es-CO')}.`);
        ...
        logError(`Mesa cerrada y cobrada: $${parseFloat(total).toLocaleString('es-CO')}`);

    } catch (e) {
        console.error(`Error al cerrar la sesión de mesa ${sessionId}: ${e.message}`);
        logError(`Error al cerrar la mesa.`);
    }
};
...
const loadProducts = async () => {
    ...
    try {
        ...
        const el = document.getElementById('products-list');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar productos: ${e.message}</p>`;
...
const attachProductListeners = () => {
    ...
    document.getElementById('add-product-btn')?.addEventListener('click', () => showProductModal(null));
...
const showProductModal = (product) => {
    ...
    document.getElementById('select-image-btn')?.addEventListener('click', () => document.getElementById('image-upload-input').click());
...
const loadOrdersCaja = async () => {
    ...
    try {
        ...
        const el = document.getElementById('orders-list-caja');
        if (el) el.innerHTML = tableHTML;
        attachOrderCajaListeners();
    } catch (e) {
        const el = document.getElementById('orders-list-caja');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar órdenes pendientes: ${e.message}</p>`;
...
const generateOrderTableCaja = (data) => {
    ...
    (data || []).forEach(o => {
        html += `
            <tr class="hover:bg-gray-50">
                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">${String(o.id || '').substring(0, 8)}...</td>
                ...
                <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">$${o.total_amount || 0}</td>
                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">${o.created_at ? new Date(o.created_at).toLocaleDateString() : ''}</td>
...
const confirmOrder = async (orderId) => {
    try {
        ...
        console.log(`Orden ${String(orderId).substring(0, 8)}... confirmada y movida (por el trigger SQL).`);
    } catch (e) {
...
const rejectOrder = async (orderId) => {
    try {
        ...
        console.log(`Orden ${String(orderId).substring(0, 8)}... rechazada.`);
    } catch (e) {
...
const loadOrdersDespacho = async () => {
    ...
    try {
        ...
        const el = document.getElementById('orders-list-despacho');
        if (el) el.innerHTML = tableHTML;
        attachOrderDespachoListeners(orders);
    } catch (e) {
        const el = document.getElementById('orders-list-despacho');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar órdenes para despacho: ${e.message}</p>`;
        ...
        button.onclick = async () => {
            ...
            if (action === 'dispatch') {
                ...
                if (order) showOrderDetailModal(order);
                    ...
                    const [fresh] = await makeRequest(`${BASE_API_URL}/orders_confirmed?id=eq.${id}&select=*`) || [];
                    if (fresh) showOrderDetailModal(fresh);
                }
            }
        };
    });
...
const dispatchOrder = async (orderId) => {
    try {
        ...
        console.log(`Orden ${String(orderId).substring(0, 8)}... marcada como Despachada.`);
        loadOrdersDespacho();
    } catch (e) {
...
const showOrderDetailModal = (order) => {
    document.getElementById('modal-title').textContent = `Detalle de Orden #${String(order.id || '').substring(0, 8)}`;

    let totalItemsCount = 0;
    let subtotalItemsPrice = 0;

    let itemsArr = order.order_items || [];
    if (typeof itemsArr === 'string') {
        try { itemsArr = JSON.parse(itemsArr); } catch (_) { itemsArr = []; }
    }
...
const printOrder = (order) => {
    ...
    let itemsText = (itemsArr || []).map(item =>
    ...
    printArea.innerHTML = `
        ...
        <div class="mb-2">--------------------------------</div>
        <div>Orden ID: ${String(order.id || '').substring(0, 8)}</div>
        <div>Fecha: ${order.created_at ? new Date(order.created_at).toLocaleString() : ''}</div>
        <div>Cliente: ${order.customer_name || ''}</div>
        <div>--------------------------------</div>
        <div class="font-bold">DETALLE:</div>
        ${itemsText}
        <div>--------------------------------</div>
        <div class="text-right font-bold">TOTAL: $${order.total_amount || 0}</div>
...
const loadOrdersCartera = async () => {
    ...
    try {
        ...
        const el = document.getElementById('orders-list-cartera');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar datos contables: ${e.message}</p>`;
...
const renderOrdersCartera = (data) => {
    ...
    const totEl = document.getElementById('cartera-total');
    if (totEl) totEl.textContent = `Costo Total Vendido: $${totalVendido.toLocaleString('es-CO')}`;
};
    ...
    (data || []).forEach(o => {
        html += `
            <tr class="hover:bg-gray-50">
                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">${String(o.id || '').substring(0, 8)}...</td>
                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">${o.customer_name || ''}</td>
                <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">$${(o.total_amount || 0).toLocaleString('es-CO')}</td>
                <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">${o.created_at ? new Date(o.created_at).toLocaleDateString() : ''}</td>
                <td class="px-3 py-4 whitespace-nowrap text-sm text-orange-600">${o.order_status || ''}</td>
...
const loadOrdersPendientes = async () => {
    ...
    try {
        ...
        const el = document.getElementById('orders-list-pendientes');
        if (el) el.innerHTML = tableHTML;
...
const generatePendingOrdersTable = (data) => {
    ...
    data.forEach(o => {
        ...
        columns.forEach(col => {
            let val = o[col];
            if (col === 'id' && val) val = `${String(val).substring(0, 8)}...`;
...
const loadOutMoney = async () => {
    try {
        ...
        const el = document.getElementById('out-money-list');
        if (el) el.innerHTML = html;
        attachOutMoneyListeners(outRecords);
    } catch (e) {
        const el = document.getElementById('out-money-list');
        if (el) el.innerHTML = `<p class="text-red-500">Error al cargar egresos: ${e.message}</p>`;
...
const computeShiftSummary = async (startDate, endDate) => {
    try {
        ...
        let openTables = [];
        try {
            openTables = await makeRequest(`${BASE_API_URL}/table_sessions?select=total,customer_address,table_number,status&status=eq.open`) || [];
        } catch (err) {
            console.warn('No se pudieron obtener mesas abiertas:', err);
            openTables = [];
        }
        ...
        // Filtrar openTables por la misma regla de "mesa"
        const filteredOpenTables = (openTables || []).filter(t => isTableSessionTable(t));
        const totalOpenTables = filteredOpenTables.reduce((sum, t) => {
            const val = Number(t.total || 0);
            return sum + (isNaN(val) ? 0 : val);
        }, 0);

        const subtotalBeforeEgresos = totalSales - totalOpenTables;
        const net = subtotalBeforeEgresos - totalOut;

        return { totalSales, totalOut, net, totalOpenTables, subtotalBeforeEgresos };
    } catch (e) {
...
const loadShiftSummary = async () => {
    ...
    try {
        ...
        const end = new Date();
        const { totalSales, totalOut, net, totalOpenTables, subtotalBeforeEgresos } = await computeShiftSummary(start, end);

        document.getElementById('summary-total-sales').textContent = `Ventas: $${totalSales.toLocaleString('es-CO')}`;
        document.getElementById('summary-open-tables').textContent = `Mesas Abiertas: $${totalOpenTables.toLocaleString('es-CO')}`;
        document.getElementById('summary-subtotal-before-egresos').textContent = `Subtotal en caja: $${subtotalBeforeEgresos.toLocaleString('es-CO')}`;
    ...
    try {
        ...
        document.getElementById('modal-body').innerHTML = `
            ...
            <div class="mt-3 p-3 bg-gray-50 rounded">
                ...
                <p><strong>Mesas Abiertas:</strong> $${totalOpenTables.toLocaleString('es-CO')}</p>
                <p><strong>Subtotal en caja:</strong> $${subtotalBeforeEgresos.toLocaleString('es-CO')}</p>
...
const printShiftSummary = async () => {
    ...
    try {
        ...
        printArea.innerHTML = `
            ...
            <div>Total Mesas Abiertas: $${totalOpenTables.toLocaleString('es-CO')}</div>
            <div>Subtotal en caja: $${subtotalBeforeEgresos.toLocaleString('es-CO')}</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('apply-cartera-filters')?.addEventListener('click', applyCarteraFilters);
    document.getElementById('export-cartera-btn')?.addEventListener('click', exportCarteraToCSV);
    document.getElementById('close-modal')?.addEventListener('click', () => setView('modal', false));
